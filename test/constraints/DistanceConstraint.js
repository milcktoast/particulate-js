QUnit.module('Constraint.Distance')
...
QUnit.test('Creation', function (assert) {
  ...
  assert.equal(fromArgs._min2, min * min,
    'Should initialize precalculated min^2.')
  assert.equal(fromArgs._max2, max * max,
    'Should initialize precalculated max^2.')
  assert.equalArray(fromArgs.indices, [a, b],
...
QUnit.test('Application of distance', function (assert) {
  ...
  assert.close(dist0, dist, 0.1,
    'Should constrain single set of particles to distance.')
  assert.closeArray([dist1, dist2], [dist, dist], 0.1,
    'Should constrain multiple sets of particles to distance.')
})

QUnit.test('Application of distance range', function (assert) {
  var system = Particulate.ParticleSystem.create(6, 10)
  ...
  assert.range(system.getDistance(0, 1), 0, 2,
    'Should not affect coincident particles if lower bound is 0.')
  assert.range(system.getDistance(2, 3), 2, Infinity,
  ...
  assert.range(system.getDistance(4, 5), 2, 4,
